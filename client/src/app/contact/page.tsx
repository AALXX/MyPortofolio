...
const Contact = () => {
    ...
    // Import motion and toast only on client side
    ...
    useEffect(() => {
        ...
        // Dynamic imports
        Promise.all([import('framer-motion'), import('sonner'), import('@emailjs/browser')]).then(([motionModule, sonnerModule, emailjsModule]) => {
            setMotion(motionModule)
            setToast(sonnerModule.toast)
    ...
    const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            if (!emailjs) {
                toast?.error('Email service not loaded yet. Please try again.')
                return
            }
    ...
    // Show loading state while client-side libraries load
    if (!mounted || !motion) {
        return (
                ...
                <div className="space-y-12">
                    ...
                    <div className="mb-12 grid gap-8 md:grid-cols-3">
                        {[
                            ...
                            <div key={index} className="border-border bg-card space-y-2 rounded-xl border p-6 text-center">
                    ...
                    <form onSubmit={handleSubmit} className="border-border bg-card space-y-6 rounded-xl border p-8">
    ...
    return (
        <div className="container mx-auto max-w-4xl px-6 py-24">
            <MotionDiv initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="space-y-12">
                <div className="space-y-4 text-center">
                    <h1 className="text-5xl font-bold tracking-tight">
                        Get In <span className="text-gradient">Touch</span>
                    </h1>
                ...
                <MotionDiv initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.3 }} onSubmit={handleSubmit} as="form" className="border-border bg-card space-y-6 rounded-xl border p-8">
                    <div className="space-y-2">
                        <label htmlFor="name" className="text-foreground text-sm font-medium">
                            Name
                        </label>
